...
- **Live mode（本番モード）への移行**: Meta の **App Review 承認** が必要。審査自体は 2〜4 週間、**準備〜承認まで通算 4〜6 週間** が目安。却下されると追加で数週間ロス
...
**重要な前提**: Meta は **2025 年 1 月 27 日に旧 Instagram Graph API スコープを廃止** し、現行は **「Instagram API with Instagram Login」** が標準です。本記事の権限名は新仕様に準拠しています。

| 用途 | 必要な権限 | App Review |
|---|---|---|
| 自社アカウントの検証・テスト | （Development mode）| **不要** |
| 顧客アカウントの基本情報取得 | `instagram_business_basic` | **必要** |
| 顧客アカウントのコメント取得・返信 | `instagram_business_manage_comments` | **必要** |
| Insights データ取得 | `instagram_business_manage_insights` | **必要** |
| コンテンツ投稿（自動返信を含む）| `instagram_business_content_publish` | **必要** |
| 自社の SaaS / B2B サービス提供 | 上記すべて | **必要** |

**ポイント**:
- 「顧客の Instagram アカウントを管理する」目的なら、ほぼ確実に App Review が必要です
- 旧スコープ（`instagram_basic` / `instagram_manage_comments` 等）は **2025-01-27 で廃止済み**。古いドキュメントを参照しないよう注意
...
- `instagram_business_manage_comments`: 自社 IG でコメント取得・返信を実演
- `instagram_business_manage_insights`: Insights データ取得を実演
- `instagram_business_content_publish`: コンテンツ投稿（自動返信を含む）を実演
...
```
Use case for instagram_business_manage_comments:
...
申請準備から承認まで含めて **4〜6 週間** を見ておくのが現実的です（却下されるとさらに +2〜4 週間）。
