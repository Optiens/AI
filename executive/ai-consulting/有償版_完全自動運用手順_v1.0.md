...
本番公開は、Vercel 本番環境変数と MTG 予約URLの設定完了後に行う。未設定のまま paid 申込ガードを外さない。
...
- 宛先: 申込企業名
- 取引日/発行日/領収日
- 取引内容: 【詳細版】AI活用診断（詳細レポート + 60分オンラインMTG）
- 税抜金額: ¥5,000
- 消費税額（10%）: ¥500
- 合計（税込）: ¥5,500
- 発行者: 合同会社Optiens
...
推奨:
- `PAID_MTG_BOOKING_URL`
- `ADMIN_ALERT_EMAIL`

`PAID_MTG_BOOKING_URL` がない場合、レポート納品は自動でも MTG 日程調整がメール返信ベースになり、完全自動とは言えない。
