...
// =================================================
{
  ...
  addFooterNote(slide, '※ 概算導入費用は本レポート時点の前提に基づくレンジです。正式見積は導入支援のご相談時に実施範囲を確認して確定します');
}
...
// =================================================
{
  ...
  addFooterNote(slide, '※ 金額は本レポート時点の概算です。正式見積は導入支援のご相談時に対象業務・連携先・データ状態を確認して確定します');
}
...
// =================================================
{
  ...
  addFooterNote(slide, '※ 推奨A は中小事業者向けに最適化された構成。実装範囲・利用量により最終構成は導入支援のご相談時に決定');
}
...
// =================================================
{
  ...
  addFooterNote(slide, '※ さらに具体的なご相談が必要な場合は、AI活用レビュー面談または導入支援のご相談として別途対応します。自由記述がない場合は本ページを省略します');
}
...
// =================================================
{
  ...
  // ステップ4列
  const steps = [
    { num: '01', label: '内容確認', desc: '本レポートで導入可否・優先順位・概算費用の前提を確認' },
