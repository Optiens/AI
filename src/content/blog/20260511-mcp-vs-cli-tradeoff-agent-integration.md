...
同じ操作を MCP と CLI で実行した場合、**CLI の方が 5〜10 倍速い** ケースが珍しくありません。

理由：

- MCP は **毎回「使えるツール一覧を読む」** ためのオーバーヘッドがある
- MCP の通信が 1 操作あたり数百ミリ秒〜数秒かかる
- CLI は OS のネイティブ機能を直接呼ぶため、ミリ秒以下で完了する

たとえば「100 件のファイルを別フォルダにコピーする」という単純作業：

- CLI（`cp` コマンド）：数秒で完了
- MCP（ファイル操作 MCP 経由）：数十秒〜数分かかることがある

もちろんすべての MCP が遅いわけではありませんが、**「数百件以上の繰り返し処理」では CLI が有利** という傾向があります。
