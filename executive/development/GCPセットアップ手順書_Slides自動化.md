...
→optiens-slides-automation@optiens-slides-automation.iam.gserviceaccount.com
...
3. 仕様書の **11スライド構成** に従って手動デザイン:
   - Brand Guideline v1.0 準拠（ディープラピス #1F3A93 / 桜 #E48A95）
...
### ⚠️ 2026-05-07 追記: PPTX 自動生成版を採用（完了済）

Claude が `pptxgenjs` で 11 スライドの PPTX を完全自動生成しました:

| 項目 | 内容 |
|---|---|
| **生成済 PPTX** | `tmp/optiens-diagnosis-template-v1.0.pptx`（400KB / 11スライド） |
| **生成スクリプト** | `scripts/generate-diagnosis-pptx.mjs`（再生成可能） |
| **デザイン準拠** | Brand Guideline v1.0（ディープラピス × 桜）に完全準拠 |
| **プレースホルダー** | 全 20+ 個埋め込み済（{{customer_name}} 等） |

#### 確定 Slides ファイル（2026-05-07）

| 項目 | 値 |
|---|---|
| **テンプレ ID** | `1ZhONMotsVzDwGBd22bRyMidt0Et4MObRPRWFGXe4ZME` |
| **URL** | https://docs.google.com/presentation/d/1ZhONMotsVzDwGBd22bRyMidt0Et4MObRPRWFGXe4ZME/edit |
| **オーナー** | takahashi.daichi@optiens.com（Workspace） |
| **タイトル** | optiens-diagnosis-template-v1.0 |
| **状態** | Slides形式に変換済・編集可能 |

#### 確定 Service Account（2026-05-07）

| 項目 | 値 |
|---|---|
| **メールアドレス** | `optiens-slides-automation@optiens-slides-automation.iam.gserviceaccount.com` |
| **一意ID** | `103600614847392786330` |
| **発行済キーID** | `62ab70dcef17037226fb01c2a974b7bfdc4d2828` |
| **GCPプロジェクト** | `optiens-slides-automation`（個人GCPプロジェクト・Workspaceアカウント所有） |
| **JSONキーファイル** | ⚠️ ローカル `.env` のみで管理・絶対に Git にコミットしない |
...
1. **Drive を開く**: https://drive.google.com にログイン（個人 Gmail）
2. **PPTX をアップロード**: `c:\workspace\optiens-hydroponics\tmp\optiens-diagnosis-template-v1.0.pptx` をドラッグ＆ドロップ
3. **Google Slides に変換**:
   - アップロードしたファイルを右クリック → **「アプリで開く」** → **「Google スライド」**
   - 自動で Slides 形式に変換され、新しいファイルが Drive に保存される
   - ファイル名: 「Optiens 無料AI活用診断レポート テンプレ v1.0」（同じ）
4. **新しい Slides ファイルの ID を取得**:
   - URL: `https://docs.google.com/presentation/d/【ここがテンプレID】/edit`
5. **不要ファイルの削除**:
   - 元の PPTX ファイル（拡張子 .pptx のまま残った方）→ ゴミ箱へ
   - 旧の空テンプレ `15oU4yDmkFn21evp5c9g3Z9y1WRWEkNfW-CwJfVY7Vkk` → ゴミ箱へ
6. **環境変数に設定**:
   ```
   GOOGLE_SLIDES_TEMPLATE_ID=<新しい Slides の ID>
   ```

#### デザイン微調整が必要なら

PPTX 生成スクリプト（`scripts/generate-diagnosis-pptx.mjs`）を編集して再生成:
```bash
node scripts/generate-diagnosis-pptx.mjs
```
その後、上記手順で再アップロード。

#### Google Slides 上で直接編集も可能

変換後の Slides ファイルは普通に編集可能です。プレースホルダー（{{customer_name}}等）は
削除しないよう注意してください（Edge Function が置換に使う）。

#### 旧情報（参考）

`[GUIDE] Optiens 無料診断テンプレ スライド内容ガイド v1.0`（Google Doc）は、
当初の手作業設計用に作成したガイドです。PPTX 自動生成に切り替わったため
参照不要ですが、レイアウト確認用に残しています。
