...
function validate(diagnosis: any): string | null {
  ...
  for (const t of diagnosis.top3) {
    ...
    const rate = HOURLY_RATE[t.staff_level] ?? HOURLY_RATE.staff
  ...
  // 階層別時給合算（最低時給 staff ベースを下限とする）
  diagnosis.roi.monthly_value_yen = Math.max(
    Math.round(totalValueYen),
    diagnosis.roi.monthly_hours_saved * HOURLY_RATE.staff,
  )
