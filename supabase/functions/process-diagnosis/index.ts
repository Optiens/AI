...
function implementationFit(monthlyYen: number): {
  ...
  if (monthlyYen < 80000) {
    return {
      ...
      guidance: '月間効果額が8万円未満のため、現時点で導入支援の見積に進むより、詳細診断で効果の大きい業務を再選定する段階です。',
    }
  }
  if (monthlyYen < 150000) {
    return {
      ...
      guidance: '月間効果額が8万〜15万円のため、いきなり本格導入費を決めず、詳細診断で対象業務を絞ってから判断するのが妥当です。',
    }
  }
...
function leadSourceGuidance(source?: string, detail?: string, referrer?: string): string {
  ...
  switch (source) {
    ...
    case 'search':
      return `検索から申し込んだリードです。初見でも分かるように専門用語を抑え、最初に取り組むべき1業務と、相談チケット/詳細診断に進む判断基準を明確にしてください。${detailText}${refText}`
    case 'referral':
...
function budgetGuidance(budget?: string): string {
  switch (budget) {
    case 'under-30':
      return '30万円未満を希望しているため、無料版では本格導入ではなく相談チケット・詳細診断・既存ツール活用の範囲に絞ってください。導入支援の正式見積額は出さず、相談チケットで小さく回収する方針を示してください。'
    case '30-60':
...
- cost_breakdown: 無料版で伝えてよい次ステップ費用、導入支援の概算帯、費用が変動する確認項目 3〜5件
  例: { category: "詳細診断+60分MTG", amount: "¥5,500(税込)", basis: "導入支援前の確認枠" }
  例: { category: "相談チケット", amount: "¥3万円/枚〜(税別)", basis: "30万円未満の相談・小規模作業" }
...
- 導入支援の正式見積は、詳細診断または個別相談で対象業務・連携先・データ状態を確認してから提示する
...
function buildReplacements(lead: any, d: any) {
  ...
  const freeCostRows = [
    { category: '詳細診断+60分MTG', amount: '¥5,500(税込)', basis: '導入支援前の確認枠。業務量と導入可否を整理' },
    { category: '相談チケット', amount: '¥3万円/枚〜(税別)', basis: '30万円未満の相談・小規模作業はこちらで対応' },
  ...
  const map: Record<string, string> = {
    ...
    // 想定コスト
    '{{cost_total_range}}': '相談チケット¥3万円〜／導入支援¥30万円〜目安',
    ...costFields,
...
<p style="font-size:13px;color:#666;">
  ...
  詳細診断 + 60分MTG（¥5,500税込）をご利用ください。導入支援に進まれた場合、本費用は初期費用に充当します。
</p>
