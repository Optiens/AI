...
本番運用では、Vercel 本番環境変数と Supabase Secrets が設定済みであることを継続確認する。設定不足が見つかった場合は、申込停止ではなく管理画面で `manual_review` に落として復旧または返金を優先する。
...
10. `/free-diagnosis` の詳細版タブから申込できることを確認する
